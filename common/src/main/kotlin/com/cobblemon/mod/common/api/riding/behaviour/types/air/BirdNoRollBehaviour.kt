...
class BirdNoRollBehaviour : RidingBehaviour<BirdNoRollSettings, BirdNoRollState> {
    companion object {
        val KEY = cobblemonResource("air/birdnoroll")
    }

    override val key: ResourceLocation = KEY

    override fun getRidingStyle(settings: BirdNoRollSettings, state: BirdNoRollState): RidingStyle {
        return RidingStyle.AIR
    }

    val poseProvider = PoseProvider<BirdNoRollSettings, BirdNoRollState>(PoseType.HOVER)
        .with(PoseOption(PoseType.FLY) { _, state, _ -> state.rideVelocity.get().z > 0.2 })

    override fun isActive(settings: BirdNoRollSettings, state: BirdNoRollState, vehicle: PokemonEntity): Boolean {
        return true
    }

    override fun pose(settings: BirdNoRollSettings, state: BirdNoRollState, vehicle: PokemonEntity): PoseType {
    ...
    */
    fun calculateRideSpaceVel(settings: BirdNoRollSettings, state: BirdNoRollState, vehicle: PokemonEntity, driver: Player) {
        //retrieve stats
    ...
    override fun rotationOnMouseXY(
        ...
        //TODO: hook up handling in parity with normal bird
        val handling = vehicle.runtime.resolveDouble(settings.handlingExpr)
        val topSpeed = vehicle.runtime.resolveDouble(settings.speedExpr)
        ...
        val yawForce =  xInput * ( 1.0 - min(sqrt(normalizeVal(state.rideVelocity.get().length(), 0.0, topSpeed)), 0.5))
    ...
    */
    ...
    override fun inertia(settings: BirdNoRollSettings, state: BirdNoRollState, vehicle: PokemonEntity): Double {
        return 0.5
    }

    override fun shouldRoll(settings: BirdNoRollSettings, state: BirdNoRollState, vehicle: PokemonEntity): Boolean {
        return true
    }

    override fun turnOffOnGround(settings: BirdNoRollSettings, state: BirdNoRollState, vehicle: PokemonEntity): Boolean {
        return false
    }

    override fun dismountOnShift(settings: BirdNoRollSettings, state: BirdNoRollState, vehicle: PokemonEntity): Boolean {
        return false
    }
    ...
    override fun createDefaultState(settings: BirdNoRollSettings) = BirdNoRollState()
}

class BirdNoRollSettings : RidingBehaviourSettings {
    override val key = BirdNoRollBehaviour.KEY
...
class BirdNoRollState : RidingBehaviourState() {
    ...
    override fun toString(): String {
        return "BirdNoRollState(rideVelocity=${rideVelocity.get()}, stamina=${stamina.get()}, gliding=${gliding.get()})"
    }

    override fun copy() = BirdNoRollState().also {
    ...
    override fun shouldSync(previous: RidingBehaviourState): Boolean {
        if (previous !is BirdNoRollState) return false
